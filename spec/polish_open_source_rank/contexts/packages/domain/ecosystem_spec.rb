...
RSpec.describe PolishOpenSourceRank::Contexts::Packages::Domain::Ecosystem do
  it 'owns supported package ecosystems' do
    ...
    expect(described_class.snapshot_supported).to eq(%w[npm rubygems crates pypi hex packagist go homebrew nuget maven])
    ...
    expect(described_class.snapshot_supported_list).to eq(
      'npm, rubygems, crates, pypi, hex, packagist, go, homebrew, nuget, maven'
    )
  end
end
