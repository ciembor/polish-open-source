...
RSpec.describe PolishOpenSourceRank::Contexts::Ranking::Domain::RankingPolicy do
  it 'owns ranking metrics, limits, tie breakers, and trending semantics' do
    ...
    expect(described_class::ORGANIZATION_RANKINGS.fetch(:active).column).to eq('merged_pull_requests_count')
    ...
    expect(described_class.bounded_limit('1000; DROP TABLE users')).to eq(101)
