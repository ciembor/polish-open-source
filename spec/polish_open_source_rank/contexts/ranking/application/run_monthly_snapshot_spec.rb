...
RSpec.describe PolishOpenSourceRank::Contexts::Ranking::Application::RunMonthlySnapshot do
  ...
  it 'runs only the organization pipeline when scoped to organizations' do
    ...
    build_job(store: store, sources: [organization_source], catalog: catalog, logger: StringIO.new).call(
      period,
      scope: :organizations
    )
  ...
  it 'refreshes merged pull requests for existing users without rerunning discovery' do
    ...
    build_job(store: store, sources: [github], catalog: catalog, logger: StringIO.new).call(
      period,
      existing_only: true,
  ...
  it 'processes existing pending user candidates before discovering more' do
    ...
    build_job(store: store, sources: [source], catalog: catalog, logger: StringIO.new).call(period)
  ...
  it 'can process existing pending user candidates without discovering more' do
    ...
    build_job(store: store, sources: [source], catalog: catalog, logger: StringIO.new).call(
      period,
      existing_only: true
    )
  ...
  it 'checks already snapshotted candidates within their source platform' do
    ...
    build_job(store: store, sources: [gitlab], catalog: catalog, logger: StringIO.new).call(period)
  ...
  it 'uses production defaults for catalog and logger' do
    expect do
      build_job(store: store, sources: [github]).call(period)
  ...
  it 'stores Codeberg profiles through the normalized source contract' do
    ...
    build_job(store: store, sources: [codeberg], catalog: catalog, logger: StringIO.new).call(period)
  ...
  it 'records candidate and run failures for retryable candidate crashes' do
    ...
    expect do
      build_job(store: store, sources: [github], catalog: catalog, logger: logger).call(period)
    end.not_to(
      raise_error
    )
    expect(store.pending_candidates(period)).to be_empty
  ...
  it 'retries failed candidates once before failing the run' do
    ...
    build_job(store: store, sources: [github], catalog: catalog, logger: StringIO.new).call(period)
  ...
  it 'discovers sources in parallel and writes platform-prefixed logs' do
    ...
    thread = Thread.new do
      build_job(store: store, sources: sources, catalog: catalog, logger: logger).call(period)
    end
  ...
  it 'processes a source without waiting for slower source discovery to finish' do
    ...
    gitlab.repositories = { 'bob' => [repository(20, 'bob/tool', 5)] }
    thread = Thread.new do
      build_job(store: store, sources: [slow_github, gitlab], catalog: catalog, logger: StringIO.new)
        .call(period)
    end
  ...
  it 'accepts loggers without flush support' do
    expect do
      build_job(store: store, sources: [github], catalog: catalog, logger: NoFlushLogger.new).call(period)
  ...
  it 'serializes candidate discovery writes from parallel sources' do
    ...
    thread = Thread.new do
      build_job(store: guarded_store, sources: sources, catalog: catalog, logger: StringIO.new).call(period)
    end
  ...
  it 'fails the run when every source fails before processing can continue' do
    ...
    expect do
      build_job(store: store, sources: sources, catalog: catalog, logger: StringIO.new).call(period)
  ...
  it 'does not record run failure when the run cannot be created' do
    ...
    expect do
      build_job(store: failing_store, sources: [github], catalog: catalog, logger: StringIO.new)
        .call(period)
  ...
  def job
    build_job(store: store, sources: [github], catalog: catalog, logger: StringIO.new)
  end

  def build_job(store:, sources:, catalog: PolishOpenSourceRank::Contexts::Ranking::Domain::LocationCatalog,
                logger: $stdout, work_events: nil)
    work_events ||= PolishOpenSourceRank::Contexts::Operations::Application::JobWorkEventRecorder.new
    monthly_logger = PolishOpenSourceRank::Contexts::Ranking::Application::MonthlySnapshotLogger.new(logger)
    store_mutex = Mutex.new
    profile_writer = monthly_profile_snapshot_writer(store, store_mutex, work_events)

    described_class.new(
      store: store,
      sources: sources,
      source_runner: monthly_source_snapshot_runner(
        store: store,
        sources: sources,
        catalog: catalog,
      ...
      source_metric_backfill: PolishOpenSourceRank::Contexts::Ranking::Application::MonthlySourceMetricBackfill.new(
        store: store,
        sources: sources,
        logger: monthly_logger,
        work_events: work_events
  ...
  def monthly_profile_snapshot_writer(store, store_mutex, work_events)
    snapshot_factory = PolishOpenSourceRank::Contexts::Ranking::Application::MonthlySnapshotFactory.new
    PolishOpenSourceRank::Contexts::Ranking::Application::MonthlyProfileSnapshotWriter.new(
      store: store,
      store_mutex: store_mutex,
      snapshot_factory: snapshot_factory,
      repository_collector: PolishOpenSourceRank::Contexts::Ranking::Application::MonthlyRepositorySnapshotCollector.new(
        store: store,
        store_mutex: store_mutex,
        work_events: work_events,
        snapshot_factory: snapshot_factory
  ...
  def monthly_source_snapshot_runner(dependencies)
    PolishOpenSourceRank::Contexts::Ranking::Application::MonthlySourceSnapshotRunner.new(
      store: dependencies.fetch(:store),
      sources: dependencies.fetch(:sources),
      logger: dependencies.fetch(:logger),
      candidate_discovery: monthly_candidate_discovery(
        dependencies.fetch(:store),
        dependencies.fetch(:catalog),
        dependencies.fetch(:logger),
        dependencies.fetch(:store_mutex)
      ),
      candidate_processors: monthly_candidate_processors(
        dependencies.fetch(:store),
        dependencies.fetch(:store_mutex),
        dependencies.fetch(:profile_writer),
        dependencies.fetch(:logger),
        dependencies.fetch(:work_events)
      ),
      store_mutex: dependencies.fetch(:store_mutex)
    )
  end

  def monthly_candidate_discovery(store, catalog, logger, store_mutex)
    PolishOpenSourceRank::Contexts::Ranking::Application::MonthlyCandidateDiscovery.new(
      store: store,
      catalog: catalog,
      logger: logger,
      store_mutex: store_mutex
  ...
  def monthly_candidate_processors(store, store_mutex, profile_writer, logger, work_events)
    processors = PolishOpenSourceRank::Contexts::Ranking::Application::MonthlySourceSnapshotRunner::CandidateProcessors
    processors.new(
      user: monthly_user_candidate_processor(store, store_mutex, profile_writer, logger, work_events),
      organization: monthly_organization_candidate_processor(store, store_mutex, profile_writer, logger, work_events)
    )
  end

  def monthly_user_candidate_processor(store, store_mutex, profile_writer, logger, work_events)
    PolishOpenSourceRank::Contexts::Ranking::Application::MonthlyUserCandidateProcessor.new(
      **candidate_processor_dependencies(store, store_mutex, logger, work_events),
      profile_writer: profile_writer
    )
  end

  def monthly_organization_candidate_processor(store, store_mutex, profile_writer, logger, work_events)
    PolishOpenSourceRank::Contexts::Ranking::Application::MonthlyOrganizationCandidateProcessor.new(
      **candidate_processor_dependencies(store, store_mutex, logger, work_events),
      profile_writer: profile_writer
    )
  end

  def candidate_processor_dependencies(store, store_mutex, logger, work_events)
    {
      store: store,
      store_mutex: store_mutex,
      classifier: PolishOpenSourceRank::Contexts::Ranking::Domain::LocationClassifier.new,
      logger: logger,
      work_events: work_events
    }
  end
