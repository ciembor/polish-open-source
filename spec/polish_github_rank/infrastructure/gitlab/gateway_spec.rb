...
class FakeGitLabClient
  ...
  def queue(body:, next_page: nil)
    @responses << PolishGithubRank::Infrastructure::GitLabClient::Response.new(
      status: 200,
...
RSpec.describe PolishGithubRank::Infrastructure::GitLabGateway do
  ...
  it 'translates missing users to the source contract error' do
    client.queue_error(PolishGithubRank::Infrastructure::GitLabClient::NotFound.new('missing',
                                                                                    status: 404,
                                                                                    body: '{}'))

    expect { gateway.user('missing', 404) }.to raise_error(PolishGithubRank::Application::SourceNotFound)
  end
  ...
  it 'uses zero for unsupported GitLab monthly star deltas and missing activity' do
    ...
    client.queue_error(PolishGithubRank::Infrastructure::GitLabClient::NotFound.new('missing', status: 404, body: '{}'))
