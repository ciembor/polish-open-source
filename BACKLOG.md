...
- [ ] Rozdzielić monitor na niezależne sekcje:
  - [ ] monthly users,
  - [ ] monthly organizations,
  - [ ] user repositories,
  - [ ] organization repositories,
  - [ ] package repository scans,
  - [ ] package manifests,
  - [ ] registry packages,
  - [ ] registry snapshots per ecosystem.
- [ ] Dodać jawny model etapów joba, zamiast wyprowadzać wszystko pośrednio z `sync_runs`, `crawl_job_runs` i istniejących tabel rankingowych.
- [ ] Notować zdarzenia przetwarzania w osobnej tabeli operacyjnej, np. `job_work_events`:
  - [ ] `job_run_id` albo stabilny identyfikator runu,
  - [ ] `period_start`,
  - [ ] `job_kind`,
  - [ ] `stage`,
  - [ ] `unit_kind`,
  ...
  - [ ] `duration_ms`,
  - [ ] `error`.
- [ ] Utrzymać istniejące tabele domenowe jako źródło prawdy dla wyników, a tabelę zdarzeń traktować jako obserwowalność/telemetrię.
- [ ] Mierzyć osobno średni, medianowy i p95 czas:
  - [ ] przetworzenia kandydata-usera,
  - [ ] przetworzenia kandydata-organizacji,
  - [ ] pobrania i zapisania repozytoriów usera,
  - [ ] pobrania i zapisania repozytoriów organizacji,
  - [ ] policzenia delty gwiazdek repozytorium,
  - [ ] zeskanowania repozytorium packages,
  - [ ] sparsowania manifestu,
  - [ ] rozwiązania pakietu w registry,
  - [ ] pobrania snapshotu registry package.
- [ ] Dla każdego etapu pokazywać:
  - [ ] total,
  - [ ] done,
  - [ ] pending,
  - [ ] failed,
  ...
  - [ ] medianę,
  - [ ] p95,
  - [ ] ETA według średniej i według p95.
- [ ] Dla packages pokazywać osobne wiersze per ecosystem, żeby npm/crates/RubyGems/PyPI/Packagist/Go nie zasłaniały się nawzajem.
- [ ] Dla package repository scans rozdzielić `repository_kind=user` i `repository_kind=organization`.
- [ ] Dla registry snapshots rozdzielić statusy `active`, `not_found`, `rate_limited`, `failed`, `pending`.
- [ ] Pokazywać, który stage jest aktualnie wykonywany i kiedy ostatnio przesunął licznik.
- [ ] Dodać ostrzeżenie stale progress, jeśli dany etap nie ma nowych zdarzeń przez konfigurowalny czas.
- [ ] Dodać linki lub drill-down do ostatnich błędów per stage i ecosystem.
- [ ] Zachować `/internal/jobs` jako noindex/no-store i nie wystawiać danych operacyjnych do publicznych cache.
- [ ] Dodać testy read modelu monitora bez realnej sieci.
- [ ] Dodać testy, że scoped monthly jobs pokazują userów i organizacje niezależnie.
- [ ] Dodać testy, że package job pokazuje metryki per ecosystem i per stage.
- [ ] `bin/quality` przechodzi.
- [ ] Zmiany są commitowane po przejściu hooków.
