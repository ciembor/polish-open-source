    ...
    class SQLiteRepositoryJobProgressSections
      MONTHLY_REPOSITORY_STAGES = [
        ...
        { label: 'organization repositories', stage: 'organization_repository',
          stats_table: :organization_monthly_stats, repository_stats_table: :organization_repository_monthly_stats }
      ].freeze
      ...
      def total_repositories(stats_table, period_start, platform)
        database.dataset(stats_table).where(period_start: period_start, platform: platform).sum(:public_repo_count).to_i
      end
