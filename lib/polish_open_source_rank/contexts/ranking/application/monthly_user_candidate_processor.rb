      ...
      module Application
        class MonthlyUserCandidateProcessor < MonthlyCandidateProcessor
          ...
          def process_request(request)
            ...
            profile_writer.record_contributor(request.accepted_profile(profile_writer, profile, location))
          ...
          def processed_candidate?(request)
            ...
            with_store { store.processed_user?(request.period, request.platform, request.source_id) }
          end

          def mark_processed(request)
            with_store { request.mark_user_candidate(store, 'processed') }
            'processed'
          end

          def mark_missing(request)
            with_store { request.mark_user_candidate(store, 'missing') }
            'missing'
          end

          def mark_rejected(request)
            with_store { request.mark_user_candidate(store, 'rejected') }
            'rejected'
          end

          def mark_failed(request, error)
            error_message = "#{error.class}: #{error.message}"
            with_store { request.mark_user_candidate(store, 'failed', error_message) }
            logger.puts "[#{request.platform}] candidate #{request.login.inspect} failed: #{error_message}"
            'failed'
          end
        end
      end
